...
## 화면(탭) 구성
1. **전형 현황(대시보드)**: 단계 요약 칩(필터), **채용 현황표**(단계별 전형 목록 표 — `renderStatusBoard`), 부서 카드(진행중만), 카드 클릭→전형 상세 보드. 카드에 체크리스트 기반 "지금 진행할 차례" 자동 표시 + 비고. **카드는 그립(⠿) 드래그로 순서 변경**(`moveDept`, `state.departments` 배열 재정렬 후 저장 — 관리자·일반 화면 공통).
