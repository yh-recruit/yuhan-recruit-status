...
### 2026-07-04 유지보수 라운드 (기술 결함 정리)
- 면접 시간 정렬 오류(9:30이 14:00 뒤로) 수정, 클라우드 오염 데이터 방어, 조회 계정 쓰기 차단, 날짜·시간 유효성 검증, 세션 유지(새로고침 시 재로그인 제거), 공휴일 데이터(2027 일부) 보강 등. 상세는 `tasks/사이트전체점검/` 참고.
