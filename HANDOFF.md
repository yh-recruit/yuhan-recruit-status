...
> ⚠️ **이 문서는 초기 인수인계용 구버전입니다(단계명·데이터 모델 일부 오래됨).**
> 현재 기준 최신 기술 문서는 **`B팀(개발)/CLAUDE.md`**(데이터 모델·화면 구성·클라우드 동기화 구조)이며, 충돌 시 CLAUDE.md가 우선합니다.
> 예: 단계명은 붙여쓰기(`공고진행`)로 통일, 클라우드는 `app/state` 문서를 필드 분해 저장(부분 업데이트). 아래 본문의 옛 표기(`공고 진행` 등)는 참고용.
